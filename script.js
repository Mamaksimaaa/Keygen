...
const IPS_PATH     = "/ip_locks"; // Хранение IP-блокировок
...
// Хешируем IP, чтобы не хранить в открытом виде
async function hashIP(ip) {
    ...
    return Array.from(new Uint8Array(hash))
        ...
        .join("")
        .slice(0, 24); // Берём первые 24 символа
}
...
async function checkExistingKey() {
    ...
    try {
        ...
        // Проверяем, есть ли уже выданный ключ для этого IP
        ...
        if (ipLock && ipLock.key && ipLock.expires_at) {
            ...
            if (now < ipLock.expires_at) {
                // Ключ ещё активен — показываем его
                showExistingKey(ipLock.key, ipLock.expires_at);
                return;
            }
        }

        // Ключа нет или истёк — показываем кнопку получения
        showState("ready");
    } catch (err) {
        console.error(err);
...
function showExistingKey(key, expiresAt) {
    ...
    const updateTime = () => {
        ...
        if (left <= 0) {
            checkExistingKey(); // Обновляем состояние
            return;
        }
